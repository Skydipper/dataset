...
describe('Find by ids datasets', () => {
    ...
    it('Return only existing datasets', async () => {
        ...
        response.body.data[0].id.should.equal(cartoFakeDataset.id);
    })

    it('Return 404 error if no data provided at all', async () => {
        const response = await requester.post(`${BASE_URL}/find-by-ids`).send();
